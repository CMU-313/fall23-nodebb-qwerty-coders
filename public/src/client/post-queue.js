    ...
    PostQueue.init = function () {
        ...
        $('.posts-list').on('click', '[data-action]', async function () {
            ...
            if (
                ...
                (action === 'reject' &&
                    !(await confirmReject('[[post-queue:confirm-reject]]')))
            ) {
                return;
            }
            ...
            socket.emit(
                'posts.' + action,
                {
                    id: id,
                    message:
                        action === 'notify' ? await getMessage() : undefined,
                },
                function (err) {
                    if (err) {
                        ...
                        Promise.all([
                            ...
                            .then(function (result) {
                                ...
                                app.parseAndTranslate(
                                    ...
                                    function (html) {
                                        if (
                                            $this.find('.category-text').length
                                        ) {
                                            $this
                                                .find('.category-text')
                                                .text(
        ...
        $('[component="post-queue/bulk-actions"]').on(
            ...
            async function () {
                ...
                if (
                    ...
                    (showConfirm &&
                        !(await confirmReject(
                            `[[post-queue:${bulkAction}-confirm, ${ids.length}]]`
                        )))
                ) {
                    return;
                }
                ...
                const promises = ids.map((id) =>
                    socket.emit('posts.' + action, { id: id })
                );
