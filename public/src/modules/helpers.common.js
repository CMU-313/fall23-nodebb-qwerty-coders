...
module.exports = function (utils, Benchpress, relative_path) {
    ...
    function renderEvents(events) {
        return events.reduce((html, event) => {
            html += `<li component="topic/event" class="timeline-event" data-topic-event-id="${
                event.id
            }" data-topic-event-type="${event.type}">
            ...
            if (event.user) {
                if (!event.user.system) {
                    html += `<span><a href="${relative_path}/user/${
                        event.user.userslug
                    }">${buildAvatar(event.user, 'xs', true)}&nbsp;${
                        event.user.username
                    }</a></span>&nbsp;`;
                } else {
