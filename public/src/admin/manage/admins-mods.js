    ...
    AdminsMods.init = function () {
        autocomplete.user($('#admin-search'), function (ev, ui) {
            socket.emit(
                ...
                function (err) {
                    ...
                    alerts.success(
                        '[[admin/manage/users:alerts.make-admin-success]]'
                    );
                    $('#admin-search').val('');

                    if (
                        $(
                            '.administrator-area [data-uid="' +
                                ui.item.user.uid +
                                '"]'
                        ).length
                    ) {
                    ...
                    if (confirm) {
                        socket.emit(
                            ...
                            function (err) {
                                ...
                                alerts.success(
                                    '[[admin/manage/users:alerts.remove-admin-success]]'
                                );
                                userCard.remove();
                            }
                        );
        ...
        autocomplete.user($('#global-mod-search'), function (ev, ui) {
            api.put('/groups/global-moderators/membership/' + ui.item.user.uid)
                .then(() => {
                    alerts.success(
                        '[[admin/manage/users:alerts.make-global-mod-success]]'
                    );
                    $('#global-mod-search').val('');

                    if (
                        $(
                            '.global-moderator-area [data-uid="' +
                                ui.item.user.uid +
                                '"]'
                        ).length
                    ) {
            ...
            function () {
                ...
                bootbox.confirm(
                    ...
                    function (confirm) {
                        if (confirm) {
                            api.del(
                                '/groups/global-moderators/membership/' + uid
                            )
                                .then(() => {
                                    alerts.success(
                                        '[[admin/manage/users:alerts.remove-global-mod-success]]'
                                    );
                                    userCard.remove();
                                    if (
                                        !$('.global-moderator-area').children()
                                            .length
                                    ) {
                                        $(
                                            '#no-global-mods-warning'
                                        ).removeClass('hidden');
                                    }
                                })
                                .catch(alerts.error);
        ...
        autocomplete.user($('.moderator-search'), function (ev, ui) {
            ...
            api.put(
                ...
                function (err) {
                    ...
                    alerts.success(
                        '[[admin/manage/users:alerts.make-moderator-success]]'
                    );
                    input.val('');
                    ...
                    app.parseAndTranslate(
                        ...
                        function (html) {
                            $(
                                '.moderator-area[data-cid="' + cid + '"]'
                            ).prepend(html);
                            $(
                                '.no-moderator-warning[data-cid="' + cid + '"]'
                            ).addClass('hidden');
                        }
                    );
                }
                    ...
                    if (confirm) {
                        api.delete(
                            ...
                            function (err) {
                                ...
                                alerts.success(
                                    '[[admin/manage/users:alerts.remove-moderator-success]]'
                                );
                                userCard.remove();
                                if (!moderatorArea.children().length) {
                                    $(
                                        '.no-moderator-warning[data-cid="' +
                                            cid +
                                            '"]'
                                    ).removeClass('hidden');
                                }
