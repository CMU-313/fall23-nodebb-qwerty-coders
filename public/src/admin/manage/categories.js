        ...
        Benchpress.render('admin/partials/categories/create', {}).then(
            function (html) {
                ...
                const parentSelector = categorySelector.init(
                    modal.find(
                        '#parentCidGroup [component="category-selector"]'
                    ),
                    options
                );
                const cloneFromSelector = categorySelector.init(
                    modal.find(
                        '#cloneFromCidGroup [component="category-selector"]'
                    ),
                    options
                );
                function submit() {
        ...
        Promise.all(
            cids.map((cid) =>
                api
                    ...
                    .then(() => {
                        const categoryEl = listEl.querySelector(
                            `li[data-cid="${cid}"]`
                        );
                        categoryEl.classList[disabled ? 'add' : 'remove'](
                            'disabled'
                        );
                        $(categoryEl)
    ...
    function itemDragDidEnd(e) {
        ...
        if (
            ...
            if (isCategoryUpdate) {
                ...
                const oldParentCid = parseInt(
                    e.from.getAttribute('data-cid'),
                    10
                );
                const newParentCid = parseInt(
                    e.to.getAttribute('data-cid'),
                    10
                );
