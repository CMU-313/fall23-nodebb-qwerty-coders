...
define('admin/manage/privileges', [
    ...
    Privileges.setupPrivilegeTable = function () {
        $('.privilege-table-container').on(
            ...
            function () {
                ...
                const isPrivate = parseInt(
                    $rowEl.attr('data-private') || 0,
                    10
                );
                const isGroup = $rowEl.attr('data-group-name') !== undefined;
                const isBanned =
                    (isGroup &&
                        $rowEl.attr('data-group-name') === 'banned-users') ||
                    $rowEl.attr('data-banned') !== undefined;
                const sourceGroupName = isBanned
                    ? 'banned-users'
                    : 'registered-users';
                const delta =
                ...
                if (member) {
                    if (
                        ...
                        bootbox.confirm(
                            ...
                            function (confirm) {
                                if (confirm) {
                                    ...
                                    Privileges.exposeSingleAssumedPriv(
                                        columnNo,
                                        sourceGroupName
                                    );
                                } else {
                        ...
                        privilege.endsWith('admin:admins-mods') &&
                        state
                    ) {
                        bootbox.confirm(
    ...
    Privileges.addEvents = function () {
        ...
        $privTableCon.on(
            ...
            function () {
                ...
                throwConfirmModal(
                    'copyToChildrenGroup',
                    Privileges.copyPrivilegesToChildren.bind(
                        null,
                        cid,
                        groupName
                    )
        ...
        $privTableCon.on(
            ...
            function () {
                ...
                throwConfirmModal(
                    'copyToAllGroup',
                    Privileges.copyPrivilegesToAllCategories.bind(
                        null,
                        cid,
                        groupName
                    )
    ...
    Privileges.refreshPrivilegeTable = function (groupToHighlight) {
        api.get(`/categories/${cid}/privileges`, {})
            .then((privileges) => {
                ...
                app.parseAndTranslate(tpl, { privileges, isAdminPriv }).then(
                    (html) => {
                        // Get currently selected filters
                        const btnIndices = $(
                            '.privilege-filters button.btn-warning'
                        )
                            .map((idx, el) => $(el).index())
                            .get();
                        ...
                        document
                            ...
                            .forEach((con, i) => {
                                ...
                                const lastIdx =
                                    $('.privilege-filters')
                                        .first()
                                        .find('button').length - 1;
                                const idx =
                                    btnIndices[i] === undefined
                                        ? lastIdx
                                        : btnIndices[i];
    ...
    Privileges.exposeAssumedPrivileges = function () {
        ...
        */
        ...
        const getRegisteredUsersInputSelector = (privs, i) =>
            `.privilege-table tr[data-group-name]:not([data-group-name="registered-users"],[data-group-name="banned-users"],[data-group-name="guests"],[data-group-name="spiders"]) td[data-privilege="${privs[i]}"] input, .privilege-table tr[data-uid]:not([data-banned]) td[data-privilege="${privs[i]}"] input`;
    ...
    Privileges.exposeSingleAssumedPriv = function (columnNo, sourceGroupName) {
        ...
        switch (sourceGroupName) {
            ...
            default:
                inputSelectorFn = () =>
                    `.privilege-table tr[data-group-name]:not([data-group-name="registered-users"],[data-group-name="banned-users"],[data-group-name="guests"],[data-group-name="spiders"]) td[data-privilege]:nth-child(${columnNo}) input, .privilege-table tr[data-uid]:not([data-banned]) td[data-privilege]:nth-child(${columnNo}) input`;
        }
    ...
    Privileges.setPrivilege = (member, privilege, state) =>
        api[state ? 'put' : 'delete'](
            `/categories/${
                isNaN(cid) ? 0 : cid
            }/privileges/${encodeURIComponent(privilege)}`,
            { member }
        );
            ...
            autocomplete.group(inputEl, function (ev, ui) {
                if (ui.item.group.name === 'administrators') {
                    return alerts.alert({
                        type: 'warning',
                        message:
                            '[[admin/manage/privileges:alert.admin-warning]]',
                    });
                }
    ...
    Privileges.copyPrivilegesToChildren = function (cid, group) {
        ...
        socket.emit(
            ...
            function (err) {
                ...
                alerts.success(
                    '[[admin/manage/categories:privileges.copy-success]]'
                );
            }
        );
    };
    ...
    function highlightRow() {
        if (ajaxify.data.group) {
            if (
                hightlightRowByDataAttr('data-group-name', ajaxify.data.group)
            ) {
                return;
            }
    ...
    function addGroupToCategory(group, cb) {
        ...
        const privilegeSet = ajaxify.data.privileges.keys.groups.reduce(
            function (memo, cur) {
                memo[cur] = false;
                return memo;
            },
    ...
    async function addUserToCategory(user, cb) {
        ...
        // Generate data for new row
        const privilegeSet = ajaxify.data.privileges.keys.users.reduce(
            function (memo, cur) {
                memo[cur] = false;
                return memo;
            },
    ...
    function filterPrivileges(ev) {
        const [startIdx, endIdx] = ev.target
            ...
            .split(',')
            .map((i) => parseInt(i, 10));
        const rows = $(ev.target)
            .closest('table')[0]
            .querySelectorAll('thead tr:last-child, tbody tr ');
        rows.forEach((tr) => {
            tr.querySelectorAll('td, th').forEach((el, idx) => {
                ...
                el.classList.toggle(
                    'hidden',
                    !(idx >= startIdx - offset && idx <= endIdx - offset)
                );
            });
        });
        checkboxRowSelector.updateAll();
