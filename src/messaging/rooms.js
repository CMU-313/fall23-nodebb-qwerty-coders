...
module.exports = function (Messaging) {
    ...
    Messaging.isUserInRoom = async (uid, roomId) => {
        const inRoom = await db.isSortedSetMember(
    ...
    Messaging.roomExists = async (roomId) =>
        db.exists(`chat:room:${roomId}:uids`);
    ...
    Messaging.isRoomOwner = async (uids, roomId) => {
        ...
        const isOwners = uids.map(
            (uid) => parseInt(uid, 10) === parseInt(owner, 10)
        );
        ...
        const result = await Promise.all(
            isOwners.map(async (isOwner, index) => {
                const payload = await plugins.hooks.fire(
                    ...
                    { uid: uids[index], roomId, owner, isOwner }
                );
    ...
    Messaging.addUsersToRoom = async function (uid, uids, roomId) {
        ...
        await Promise.all(
            payload.uids.map((uid) =>
                Messaging.addSystemMessage('user-join', uid, payload.roomId)
            )
        );
    };
    ...
    async function updateGroupChatField(roomIds) {
        ...
        const groupChats = roomIds.filter(
    ...
    Messaging.getUidsInRoom = async (roomId, start, stop) =>
        db.getSortedSetRevRange(`chat:room:${roomId}:uids`, start, stop);
    ...
    Messaging.renameRoom = async function (uid, roomId, newName) {
        ...
        const isOwner = await Messaging.isRoomOwner(
            payload.uid,
            payload.roomId
        );
        if (!isOwner) {
    ...
    Messaging.loadRoom = async (uid, data) => {
        ...
        const [room, canReply, users, messages, isAdminOrGlobalMod] =
            await Promise.all([
