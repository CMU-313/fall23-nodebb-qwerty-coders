...
async function registerAndLoginUser(req, res, userData) {
    ...
    if (userData.token) {
        ...
        await Promise.all([
            user.confirmIfInviteEmailIsUsed(
                userData.token,
                userData.email,
                uid
            ),
...
authenticationController.register = async function (req, res) {
    ...
    try {
        ...
        if (
            ...
            slugify(userData.username).length <
                meta.config.minimumUsernameLength
        ) {
...
async function addToApprovalQueue(req, userData) {
    ...
    if (meta.config.showAverageApprovalTime) {
        ...
        if (average_time > 0) {
            message += ` [[register:registration-queue-average-time, ${Math.floor(
                average_time / 60
            )}, ${Math.floor(average_time % 60)}]]`;
        }
    }
    ...
    try {
        ...
        const done = function (data) {
            ...
            if (data && data.message) {
                return res.redirect(
                    `${relative_path}/?register=${encodeURIComponent(
                        data.message
                    )}`
                );
            }
            ...
            if (req.session.returnTo) {
                res.redirect(
                    relative_path +
                        req.session.returnTo.replace(
                            new RegExp(`^${relative_path}`),
                            ''
                        )
                );
            } else {
        ...
        delete req.session.registration;
        res.redirect(
            `${nconf.get('relative_path')}/?register=${encodeURIComponent(
                err.message
            )}`
        );
    }
    ...
    try {
        ...
        if (isEmailLogin || isUsernameLogin) {
            ...
            errorHandler(
                req,
                res,
                `[[error:wrong-login-type-${loginWith}]]`,
                400
            );
        }
    } catch (err) {
...
authenticationController.doLogin = async function (req, uid) {
    ...
    await loginAsync(
        { uid: uid },
        { keepSessionInfo: req.res.locals !== false }
    );
...
authenticationController.localLogin = async function (
    ...
    try {
        ...
        const hasLoginPrivilege = await privileges.global.can(
            'local:login',
            uid
        );
        ...
        const passwordMatch = await user.isPasswordCorrect(
            uid,
            password,
            req.ip
        );
