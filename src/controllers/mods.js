...
modsController.flags.detail = async function (req, res, next) {
    const results = await utils.promiseParallel({
        ...
        privileges: Promise.all(
            ['global', 'admin'].map(async (type) =>
                privileges[type].get(req.uid)
            )
        ),
    });
    ...
    results.flagData.history = results.isAdminOrGlobalMod
        ? await flags.getHistory(req.params.flagId)
        : null;
...
modsController.postQueue = async function (req, res, next) {
    ...
    const [isAdmin, isGlobalMod, moderatedCids, categoriesData] =
        await Promise.all([
    ...
    res.render('post-queue', {
        ...
        ...categoriesData,
        allCategoriesUrl: `post-queue${helpers.buildQueryString(
            req.query,
            'cid',
            ''
        )}`,
